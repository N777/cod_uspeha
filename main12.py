...
class CustomEnv(gym.Env):
    ...
    def step(self, action, actor):
        ...
        if action == 0:
            ...
            #reward -= (m.sqrt((actor.rect.x+40 - WIDTH / 2)**2 + (actor.rect.y - HEIGHT / 2)**2)) * (1 / m.sqrt((WIDTH / 2) ** 2 + (HEIGHT / 2) ** 2))
            ...
            #reward -= (m.sqrt((actor.rect.x - WIDTH / 2)**2 + (actor.rect.y-40 - HEIGHT / 2)**2)) * (1 / m.sqrt((WIDTH / 2) ** 2 + (HEIGHT / 2) ** 2))
            obs = actor.states()
        elif action == 3:
...
pygame.display.set_caption("success_code+s")
...
def play_and_train(env, agents, iterp, t_max=10 ** 2*2):
    total_reward = 0.0
    global board
    board = env.reset()
...
if not train:
    game = 380
    agent1.qtable = np.loadtxt(f'weight3/sg{game}agent0.txt')
    agent2.qtable = np.loadtxt(f'weight3/sg{game}agent1.txt')
    agent3.qtable = np.loadtxt(f'weight3/sg{game}agent2.txt')
    agent4.qtable = np.loadtxt(f'weight3/sg{game}agent3.txt')
    for agent in agents:
    ...
    for i in range(1000):
        ...
        if i % 10 == 0:
            for k in range(len(agents)):
                np.savetxt(f'weight3/sg{i}agent{k}.txt', agents[k].qtable)
        print(rewards[i])

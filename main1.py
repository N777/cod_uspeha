...
if not train:
    game = 300
    agent1.qtable = np.loadtxt(f'weight3/g{game}agent0.txt')
    agent2.qtable = np.loadtxt(f'weight3/g{game}agent1.txt')
    agent3.qtable = np.loadtxt(f'weight3/g{game}agent2.txt')
    agent4.qtable = np.loadtxt(f'weight3/g{game}agent3.txt')
    for agent in agents:
    ...
    for i in range(1000):
        ...
        if i % 10 == 0:
            for k in range(len(agents)):
                np.savetxt(f'weight3/g{i}agent{k}.txt', agents[k].qtable)
        print(rewards[i])
# до сюда
